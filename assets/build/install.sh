...
# only for simplicity.
paxctl -Cm "$(command -v ruby${RUBY_VERSION})" || paxctl -cm "$(command -v ruby${RUBY_VERSION})"
# https://en.wikibooks.org/wiki/Grsecurity/Application-specific_Settings#Node.js
paxctl -Cm "$(command -v nodejs)" || paxctl -cm "$(command -v nodejs)"
...
exec_as_git bundle exec rake gitlab:assets:compile USE_DB=false SKIP_STORAGE_VALIDATION=true NODE_OPTIONS="--max-old-space-size=2048"
